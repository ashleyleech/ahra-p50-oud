...
```{r flowchart, fig.cap="* Methadone exclusion: n=0 (TennCare OTP coverage began June 2020). Follow-up >1 year exclusion: n=0 (days to event already capped at 365 days)."}                                                                                                                                                            
  ...
  grViz(paste0('
  digraph consort {
    graph [layout = dot, rankdir = TB, fontname = Helvetica, nodesep = 0.8, ranksep = 0.6, splines = ortho]

    node [shape = rectangle, fontname = Helvetica, fontsize = 11,
          style = filled, fillcolor = white, width = 4.5, height = 1]
    B1 [label = "Pregnancies with OUD evidence\\n(OUD diagnosis or MOUD prescription, BTrack=90)\\nn = ', formatC(n_step1, format="d", big.mark=","), '"]
    ...
    B4 [label = ">=1 day MOUD receipt (final analytic sample)\\nn = ', formatC(n_step6, format="d", big.mark=","), '"]
    ...
    S2 [label = "Naltrexone only or\\nwith buprenorphine\\nn = ', formatC(n_nal, format="d", big.mark=","), '"]

    B1 -> B2
    B1 -> E2 [constraint = false]
    B2 -> B3
    B2 -> E3 [constraint = false]
    B3 -> B4
    B3 -> E4 [constraint = false]
    B4 -> S1
    B4 -> S2
